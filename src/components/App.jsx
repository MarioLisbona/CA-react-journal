...
const App = () => {
  ...
  // Imitial state for categories is 'Food', 'Coding', 'Work', 'Other'
    ...
    useEffect(() => {
      async function fetchCategories() {
        const result = await fetch('http://localhost:5500/categories')
  ...
  useEffect(() => {
      ...
      const result = await fetch('http://localhost:5500/entries')
      const data = await result.json()
      setEntries(data)
      }
      fetchEntries()
  ...
  const addEntry =  async (category, content) => {
    ...
    // const categoryObject = categories.find(cat => cat.name === category)
        ...
        const returnedEntry = await fetch('http://localhost:5500/entries', {
          method: 'POST',
          headers: {
